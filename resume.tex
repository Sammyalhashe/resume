...
\newcommand{\textDateWithLoc}[4]{\noindent#1 $|$ #2 $|$ {\color{textGray} #3} $|$ {\color{textGray} #4}}
...
\begin{document}
    \begin{center}
        ...
        {\normalsize LinkedIn: \href{https://www.linkedin.com/in/sammyalhashemi}{linkedin.com/in/sammyalhashemi} $|$ GitHub: \href{https://github.com/Sammyalhashe}{https://github.com/Sammyalhashe}}\\
    ...
    \textDateWithLoc{Bloomberg}{Software Engineer}{Present}{New York City}
    \begin{small}
        \begin{itemize}
            \itemsep0em 
            ...
            \item {\color{textGray} Designed and implemented an internal pub/sub middleware delivering real-time data from backend services to multiple client-facing applications, enabling dynamic, low-latency updates across the terminal UI.}
                ...
                \end{itemize}
            \item {\color{textGray} Implemented updates to the datalayer (view) protocol of the terminal to handle updates to the pub/sub framework's protocol.}
            \item {\color{textGray} Added the ability for application developers to dynamically change metadata that subscribers send to publishers.}
                ...
                \end{itemize}
            \item {\color{textGray} Integrated routing and entitlement services into the framework, refactoring internal routing logic and adding shared libraries for RBAC/entitlement checks; improved security posture and standardized access control across 100s of applications.}
            ...
            %     \end{itemize}
            \item {\color{textGray} Owned the Cython binding for the C++ SDK of the pub/sub framework, exposing new protocol features and enabling local refroot builds instead of dpkg-only.}
            % \item {\color{textGray} Owned the Cython implementation of the pub/sub framework}
                % \begin{itemize}[label=$\circ$,topsep=-5px,partopsep=0px]
                %     \itemsep0em 
                %     \item {\color{textGray} Made it so that the Cython application could build against a local refroot instead of pulling the latest unstable from dpkg.}
                %     \item {\color{textGray} Exposed changes in the internal protocol of the c++ sdk to the Cython implementation.}
                % \end{itemize}
            % \item {\color{textGray} Modernized the settings propagation by splitting the main process into a separate task that delivers eventual consistency.}
            \item {\color{textGray} Modernized settings propagation by extracting it into a dedicated process for eventual consistency; improved fault isolation and completely eliminated config/settings related incidents.}
        \end{itemize}
    \end{small}


    \textDateWithLoc{MThree (Contracted to Morgan Stanley)}{Software Developer}{Aug 2019 -- Aug 2020}{Montreal}
    \begin{small}
        \begin{itemize}
            \itemsep0em 
            ...
            % \item {\color{textGray} Maintained a frontend javascript testing framework built with Protractor and Selenium to test new feature scenarios, reducing bug rate by close to 5\%.}
            \item {\color{textGray} Maintained E2E tests with Protractor and Selenium for new features, reducing regressions by ~5\% and accelerating releases.}
        \end{itemize}
    \end{small}

    \textDateWithLoc{Microsemi}{Device Characterization Engineer (Intern)}{Jun 2017 -- Aug 2018}{San Jose}
    \begin{small}
        \begin{itemize}
            \itemsep0em 
    ...
    \textDateWithLoc{B.S. Engineering Science (Physics)}{University of Toronto}{Sept 2014 -- June 2019}{Toronto}
    \begin{small}
        \begin{itemize}
            \itemsep0em 
    ...
    \textDate{ATP Website Scraper}{Mar 2020 -- June 2020}{\href{https://github.com/Sammyalhashe/ATPScraper}{Code URL}}
    \begin{small}
        \begin{itemize}
            \itemsep0em 
            ...
            \item {\color{textGray} As a test, also created a sample Go cli application \href{https://github.com/Sammyalhashe/atp-commandline}{here}.}
        \end{itemize}
    \end{small}
